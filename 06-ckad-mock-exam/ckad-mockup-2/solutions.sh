...
echo "Solving Problems for ckad-mockup-2..."

# 1. 무중단 롤링 업데이트 및 롤백 (Rolling Update & Rollback)
echo "Solving Problem 1..."
kubectl create deploy advanced-web -n production-tier --image=nginx:1.20 --replicas=5 || true
...
kubectl create sa audit-viewer-sa -n security-audit || true
kubectl create role audit-viewer-role -n security-audit --verb=get,list --resource=configmaps,secrets || true
kubectl create rolebinding audit-viewer-binding -n security-audit --role=audit-viewer-role --serviceaccount=security-audit:audit-viewer-sa || true

# 4. 복합 컨테이너 파드 및 공유 볼륨 (Multi-Container & Shared Volume)
echo "Solving Problem 4..."
cat <<EOF | kubectl apply -f -
apiVersion: v1
kind: Pod
metadata:
...
echo "Solving Problem 5..."
kubectl create resourcequota cpu-limit-quota -n critical-apps --hard=limits.cpu=1000m || true
cat <<EOF | kubectl apply -f -
apiVersion: v1
kind: Pod
metadata:
...
metadata:
  ...
  annotations:
    ...
    traefik.ingress.kubernetes.io/router.middlewares: multi-app-stripprefix@kubernetescrd
spec:
...
# Ingress Rewrite Middleware for Traefik
cat <<EOF | kubectl apply -f -
...
echo "Solving Problem 10..."
# Ensure local-path and standard SC allow expansion
kubectl patch sc local-path -p '{"allowVolumeExpansion": true}' || true
kubectl patch sc standard -p '{"allowVolumeExpansion": true}' || true
kubectl patch pvc data-pvc -n storage-expansion -p '{"spec": {"resources": {"requests": {"storage": "100Mi"}}}}'

# 11. 트러블슈팅: 잘못된 컨피그맵 참조 수정
echo "Solving Problem 11..."
kubectl patch deploy web-deploy -n app-stack --type='json' -p='[{"op": "replace", "path": "/spec/template/spec/containers/0/env/0/valueFrom/configMapKeyRef/key", "value": "correct-key"}]'

# 12. 보안 강화: 읽기 전용 파일시스템 적용 (ReadOnlyRootFilesystem)
echo "Solving Problem 12..."
cat <<EOF | kubectl apply -f -
apiVersion: v1
kind: Pod
metadata:
