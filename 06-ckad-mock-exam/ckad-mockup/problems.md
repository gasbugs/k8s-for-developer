...
- **요구사항:**
    ...
    2. 스케줄은 매주 월요일 새벽 2시 30분에 실행되도록 설정하십시오.
...
- **요구사항:**
    ...
    3. 빌드된 이미지를 로컬 레지스트리(`localhost:5000`)에 푸시하십시오.
    4. 빌드된 이미지를 `tool-v2.tar` 파일로 저장하십시오.
    5. **주의:** 이미지 태그는 `localhost:5000/internal-tool:v2.0` 형식을 사용해야 합니다.
...
- **요구사항:**
    ...
    3. **주의:** 설정된 Network Policy 리소스는 직접 수정하지 마십시오. 채점 시에는 `api-server`가 `database-tier`의 `database` 서비스로 통신이 성공하는지 직접 확인합니다.
...
- **상황:** `resource-mgmt` 네임스페이스에 할당된 자원 제한(ResourceQuota)으로 인해 신규 파드 생성이 실패할 수 있습니다. 운영 정책에 따라 해당 네임스페이스에 할당된 가용 자원을 확인하고 적절한 리소스를 가진 파드를 배포해야 합니다.
- **요구사항:**
    1. `resource-mgmt` 네임스페이스에 설정된 `ResourceQuota`(`mem-cpu-demo`)의 상세 설정을 확인하십시오.
    2. 확인한 쿼터의 제약 사항(hard limits)을 준수하면서, 해당 쿼터에서 허용하는 최대 리소스(또는 그에 준하는 적절한 리소스)를 할당하여 `quota-pod` 파드를 생성하십시오. (이미지: `nginx`)
    3. 파드가 정상적으로 `Running` 상태가 되어야 합니다.
...
- **요구사항:**
    ...
    3. `ingressClassName`을 `traefik`으로 지정하십시오.
    4. 실제 각 경로로의 트래픽이 정상적으로 전달되는지 확인하십시오.
